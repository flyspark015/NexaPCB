...
REF_RE = re.compile(r"^(?:[A-Z]+[0-9]+|#PWR[0-9]+)$")
...
def parse_skidl_source(source_file: str | Path) -> AstParseResult:
    ...
    helper_factories: dict[str, int | None] = {}
    ...
    while changed:
        ...
        for node in function_defs:
            if node.name in helper_factories:
                continue
            ...
            for inner in ast.walk(node):
                if isinstance(inner, ast.Assign) and isinstance(inner.value, ast.Call):
                    ...
                    if call_name == "Part" or call_name in helper_factories:
                        for target in inner.targets:
                            if isinstance(target, ast.Name):
                                created_names.add(target.id)
                elif isinstance(inner, ast.Return):
                    if isinstance(inner.value, ast.Name) and inner.value.id in created_names:
                        returns_part = True
                    elif isinstance(inner.value, ast.Call):
                        call = inner.value
                        call_name = ""
                        if isinstance(call.func, ast.Name):
                            call_name = call.func.id
            ...
            if returns_part:
                ...
                for idx, arg in enumerate(node.args.args):
                    if arg.arg == "ref":
                        ref_index = idx
                        break
                helper_factories[node.name] = ref_index
                changed = True
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Assign):
            ...
            if isinstance(node.value, ast.Call):
                ...
                elif call_name in helper_factories:
                    ref = None
                    for kw in call.keywords:
                        if kw.arg == "ref":
                            ref = _literal_str(kw.value)
                            break
                    ref_index = helper_factories.get(call_name)
                    if ref is None and ref_index is not None and len(call.args) > ref_index:
                        ref = _literal_str(call.args[ref_index])
                    if ref:
                        refs.add(ref)
    ...
    return AstParseResult(
        ...
        refs=sorted(ref for ref in refs if REF_RE.match(ref)),
        errors=errors,
    )
